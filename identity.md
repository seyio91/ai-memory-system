...
- **Delegate non-trivial *actionable* execution via the configured executor.** Pick the **role** by task nature: `--role task` (default, write-capable — a plan step) or `--role explore` (read-only scouting). Run `scripts/executor.sh --role <role> --which`:
  - prints `subagent` (or `subagent:<model>`) → delegate via the Claude `Agent` tool (use `<model>` if named, else `sonnet`; `Explore` agent type for the explore role).
  - prints `cli:<name>` → delegate by running `scripts/executor.sh --role <role> --run "<prompt>"` via Bash; if that prints `EXECUTOR_USE_SUBAGENT` (exit 3), switch to the Agent tool.
  Each role reads `AI_MEMORY_EXECUTOR_TASK` / `AI_MEMORY_EXECUTOR_EXPLORE` (`harness[:model]`) from `config.local.sh`, falling back to the legacy `AI_MEMORY_EXECUTOR` (default `claude-subagent`); the harness resolves through its manifest `exec_*` block, and an unavailable one auto-falls-back per `AI_MEMORY_EXECUTOR_FALLBACK`. A harness with no read-only mode is skipped for `explore` (degrades to Explore), never run write-capable. Research/Q&A you still handle directly; `explore` is for delegating *scoped, token-heavy* read-only scouting.
...
- **Delegate, don't load.** Do NOT pull the sibling's `memory.md` into this thread (bloats context, especially across several siblings). Delegate sibling-scoped work to the configured executor (`scripts/executor.sh --which` → `scripts/executor.sh --run "<prompt>"` or the Agent tool) with a **self-contained** prompt: point at `identity.md` (hard rules / deny-list) + `projects/<sibling>/memory.md`, state the task, default deliverable = **plan only** (no edits to the sibling repo). Keep only the returned summary in context; re-open the plan file on demand.
  - *Codex caveat:* `codex-mem.sh` builds `AGENTS.md` from the **active** project, not the sibling. Either pin the sibling repo (`.agents/memory-project`) and launch Codex there, or pass the sibling `memory.md` path in the prompt for its shell tool. A Claude subagent has no such caveat.
