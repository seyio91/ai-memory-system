...
#     and edit here). Copies under skills/<name>/ and validates. This is authoring,
#     not referencing: it does not touch the manifest. Rule of thumb: modify it ->
#     --from; just use it -> --remote.
#
...
#   install-skill.sh --from <dir|SKILL.md> [--name <name>] [--link] [--force]
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
. "$SCRIPT_DIR/_lib.sh"

FROM="" REMOTE="" REF="" RPATH="" NAME="" LINK=0 FORCE=0 SAVE=1
while [ $# -gt 0 ]; do
    case "$1" in
        --from)    FROM="${2:-}"; shift 2 ;;
...
PY
    ...
    # Validate just this skill (its own SKILL.md), then optionally link.
