...
cp "$MEM/skills/design-brainstorm/SKILL.md" "$MEM/before"
run "$AP" --skill design-brainstorm
assert_exit 0 "$code" "re-sync without --force succeeds (block present)"
set +e; cmp -s "$MEM/before" "$MEM/skills/design-brainstorm/SKILL.md"; ident=$?; set -e
assert_exit 0 "$ident" "re-sync is idempotent (byte-identical)"
n_start=$(grep -c 'partial:self-rating START' "$MEM/skills/design-brainstorm/SKILL.md")
...
assert_contains "$(cat "$MEM/skills/design-brainstorm/SKILL.md")" "partial:self-rating START" "--all kept carrier design-brainstorm"
...
run "$AP" --skill design-brainstorm --partial does-not-exist
...
printf '## notes\njust prose, no score line\n' > "$MEM/skills/design-brainstorm/self-rating.md"
run "$SR"
assert_not_contains "$out" "design-brainstorm " "scoreless log skipped in default view"
