...
2. **From `/start`, on a captured task.** When the task-provider layer exists, `/start` on a captured task that is a feature with open design delegates here. The seed is the task's pulled **summary** — treat it exactly as you would an initial user request, then run the same dialogue. (A captured task that is quick or already settled is not sent here — `/start` takes it straight to `/new-plan`.)
...
Do **not** create a separate spec document and do **not** commit anything to the repo. The design lives in the plan file, in the memory tree, like every other plan.
...
- It does not run for the executor. Brainstorming is an orchestrator activity; the orchestrator is always Claude (main session). Codex never brainstorms.
