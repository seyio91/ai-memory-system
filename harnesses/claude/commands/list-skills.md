...
Provenance is **derived**, nothing extra to maintain: a skill in `skills/` is generic authored (tracked, synced), one in `skills-local/` is local authored (gitignored, per-instance), and one in `.skill-cache/` is remote referenced (declared in root `skills.toml`, with its commit pin from `skills.lock`).
