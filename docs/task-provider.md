...
An **investigation** is the findings artifact produced while exploring, *before* a task exists. It is conditional — it exists only when an investigation was actually done and its output is too large for the summary. It serves two readers: the task summary points at it by name, and `/start` hands it to the `design-brainstorm` skill as the seed. It is **not** the brainstorm's output — that goes into the plan (`## Goal`, `## Success criteria`, `## Design`, `## Risks`), never a sibling file.

**An investigation is tied to a task's lifecycle.** As soon as the task it seeds exists, the investigation's frontmatter carries its `task_ref` — that is the lifecycle anchor (`lint-memory` flags a live investigation without one). When the task closes (done or archived) and the consuming plan has shipped, the file moves to `projects/<project>/archive/investigations/<slug>.md` — same trigger and same convention as plan archival. The by-name reference above is what makes the move free: readers resolve the slug in the live dir first, then `archive/investigations/`.

**The mechanism is `/plan-archive` itself, not a separate step.** Archiving a plan resolves its linked investigation by the plan's frontmatter `task_ref` first (matching against every live investigation's own `task_ref`), falling back to a same-slug filename match for investigations written before the `task_ref` convention landed, and moves both in the one invocation. A destination collision (`archive/investigations/<slug>.md` already exists) aborts only the investigation move — reported, not silent — and never blocks the plan's own archival. Because this depends on the archival step actually firing, `lint-memory` carries a purely local backstop: a live `investigations/<slug>.md` whose `task_ref` matches a plan already sitting in `archive/plans/` is flagged `WARN` (rule 10) — the work shipped and the investigation was left behind. The check compares frontmatter only and never calls the task provider.
...
- Captured **feature with open design** → `/start` hands the pulled summary to the [`design-brainstorm`](harnesses/claude.md#skills) skill as its seed (clarify → approaches → sectioned design); the design folds into the plan's `## Goal`/`## Design`/`## Success criteria`/`## Risks`; the linking step writes `task_provider`/`task_ref` into the plan frontmatter, pushes the brainstorm's clarified **`## Goal`** back as the refined summary via `update`, and flips status `backlog → started`.
