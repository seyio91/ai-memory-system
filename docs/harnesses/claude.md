...
| `/new-initiative <slug>` | Scaffold an active cross-project initiative from `initiatives/_template.md`; add breadcrumb rows to each affected project's `Related Projects` table. See [Initiatives](../initiatives.md). |
...
| `/start [<ref>] [--worktree]` | Begin a captured task: pull it (project-agnostic by `ref`), run the [brainstorm gate](#skills) (feature-with-open-design → `design-brainstorm` skill; settled/quick → straight to plan), scaffold the linked plan in the task's project (`task_provider`/`task_ref` frontmatter), push the clarified Goal back via `update`, flip status to `started`. Bare `/start` lists the active backlog to pick. `--worktree` (a Tier-3 feature only) enters a fresh git worktree as the final step — see below. |
