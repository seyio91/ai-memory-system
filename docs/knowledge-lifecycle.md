...
Not every skill graduates from a domain file, either. The `design-brainstorm` skill (see [Skills](harnesses/claude.md#skills)) was authored directly as a Claude-only, orchestrator-only capability — it encodes a procedure (the Tier-3-feature design pass) that never existed as cross-project *knowledge*, so it has no `domain/*.md` source and gets no index row. Authoring a skill outright is fine when the thing is a procedure from the start; the maturation path is the common case, not the only one.

**Where a skill lives is a source decision.** Independent of whether a skill graduated from a domain file, it is either **authored** in `skills/` (per-instance, gitignored except `.gitkeep`) or **remote** (referenced from a git repo in root `skills.toml`, materialized into `.skill-cache/`, pinned by the lockfile). The old generic/local authored split is retired: sharing a skill means publishing it to a remote repo and adding it to the catalog. This is how a skill is distributed rather than how it's born, so it's documented with the mechanism in [Skill Source](harnesses/claude.md#skill-source-authoredremote), not here.
