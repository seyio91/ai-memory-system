...
| `AI_MEMORY_ORCHESTRATOR` | *(unset)* | `executor.sh` — harness running the main session; names the subagent plane's family for the same-family `--which` warning. Unset: `claude` when `CLAUDECODE=1`, else `subagent` |
