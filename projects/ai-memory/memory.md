...
- **`ls`/glob output is not evidence for a count or an absence claim — corroborate with `find` or `git ls-files`.** The phantom-empty/short output that plagued this tree under `~/Downloads` **survived relocation to `~/Projects`** (2026-07-18): `ls scripts/tests/test_*.sh | wc -l` returned **1** where `find` returned **48**, on a path no longer TCC-protected. So TCC was never the whole cause — the leading (unproven) suspect is the rtk shell-filter rewrite of `ls`. The failure is silent and reads as a real result, so it fabricates confident wrong conclusions in either direction: absent-when-present *and* present-when-absent.
