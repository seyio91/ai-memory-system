...
- `fiter-infrastructure-analyzer` is migrated to its project home under `projects/<project>/skills/`, its **global `~/.claude/skills/` symlink is removed** (Personal overrides Project — leaving it would shadow the scoped copy everywhere), and the skill resolves in its target repo(s) under the chosen harness(es).
...
- **Multi-repo skills.** Pure co-location fits a single-repo skill. A skill spanning several repos (e.g. the fiter IaC analyzer, if it applies to more than one) needs a target list — handle via an optional `targets:` line in the skill's own metadata or a per-project sidecar; resolve the exact `fiter-infrastructure-analyzer` scope during Phase 3.
...
### Phase 3 — Migrate `fiter-infrastructure-analyzer` (DECIDED: no-op)
...
- `fiter-infrastructure-analyzer` scope (one repo vs many) — resolve in Phase 3; its trigger is generic, so confirm it should be scoped at all rather than left global.
