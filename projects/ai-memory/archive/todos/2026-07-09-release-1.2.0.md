...
- [x] Phase 1 — `scripts/deny-match.sh`: tokenized + adjacency matchers, `sh -c` recursion
- [x] Phase 2 — `json_parser_available()`; `pretooluse.sh` fails closed for guarded roles
- [x] Phase 3 — specs + `helm uninstall`/`delete`; gitignored `deny-list.local.txt` overlay
- [x] Phase 4 — tests: allow/deny table, fail-closed, overlay, dirty-tracked guard
- [x] Phase 5 — docs: antigravity.md §Enforcement, deny-list header, CHANGELOG (Fixed)
