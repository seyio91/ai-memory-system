...
- [ ] Phase 1 — scope foundation: `skills-local/` folder + centralized `_lib.sh:list_skill_dirs` (link-skills + validate-skills route through it; `/skills-local/` gitignored)
- [ ] Phase 2 — authored authoring + migrate: `new-skill --local`, migrate fiter, route boundary/ratings/partial through the enumerator
- [ ] Phase 3 — remote source layer: split manifests + resolver → gitignored `.skill-cache/` (+ lockfile), cache as a third enumeration root
- [ ] Phase 4 — remote authoring + sync: `install-skill --remote`, `sync-system` resolve/update step, offline degradation
- [ ] Phase 5 — tests (2×2 taxonomy) + docs
